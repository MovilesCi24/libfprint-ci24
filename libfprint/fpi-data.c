...
static void storage_setup(void)
{
	...
	base_store = g_build_filename("/free/CI24/", "fprint/prints", NULL);
	...
	enum fp_finger finger)
{
	...
	g_snprintf(fingername, 4, "%i", finger);
...
static char *__get_path_to_print_code(uint16_t driver_id, uint32_t devtype, int finger_code)
{
	char *dirpath;
	char *path;
	char fingername[4];

	g_snprintf(fingername, 4, "%i", finger_code);
...
static char *get_path_to_print_code(struct fp_dev *dev, int finger_code)
{
	return __get_path_to_print_code(dev->drv->id, dev->devtype, finger_code);
}
 ...
 */
API_EXPORTED int fp_print_data_save(struct fp_print_data *data, int finger_code)
{
	GError *err = NULL;
	char *path;
	char *dirpath;
	...
	fp_dbg("save %i print from driver %04x", finger_code, data->driver_id);
	len = fp_print_data_get_data(data, &buf);
	if (!len)
		return -ENOMEM;

	path = __get_path_to_print_code(data->driver_id, data->devtype, finger_code);
 ...
 * Returns: 0 on success, non-zero on error
 */
API_EXPORTED int fp_print_data_load(struct fp_dev *dev, int finger_code, struct fp_print_data **data)
{
	gchar *path;
	...
	path = get_path_to_print_code(dev, finger_code);
 ...
 */
API_EXPORTED int fp_print_data_delete(struct fp_dev *dev, int finger_code)
{
	int r;
	gchar *path = get_path_to_print_code(dev, finger_code);

	fp_dbg("remove finger %d at %s", finger_code, path);
	r = g_unlink(path);
	g_free(path);
	if (r < 0)
